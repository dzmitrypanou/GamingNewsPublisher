...
pub fn get_automation_status(
    ...
    Ok(AutomationStatus {
        ...
        fetch_schedule_start_at: settings.fetch_schedule_start_at.clone(),
        fetch_repeat_unit: settings.fetch_repeat_unit.clone(),
        fetch_repeat_every: settings.fetch_repeat_every,
        next_fetch_at: state.fetch_scheduler_runtime.next_fetch_at(),
        fetch_scheduled_delay_seconds: state.fetch_scheduler_runtime.scheduled_delay_secs(),
        ...
        backup_schedule_start_at: settings.backup_schedule_start_at.clone(),
        backup_repeat_unit: settings.backup_repeat_unit.clone(),
        backup_repeat_every: settings.backup_repeat_every,
        backup_directory: settings.backup_directory.clone(),
        last_backup_at: state.db.get_last_backup_at().ok().flatten(),
        next_backup_at: state.backup_scheduler_runtime.next_backup_at(),
        backup_scheduled_delay_seconds: state.backup_scheduler_runtime.scheduled_delay_secs(),
    })
}
